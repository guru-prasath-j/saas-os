...
amy/
  ...
  finance/
    ...
    categorizer.py     — Rule-based + regex categorizer (instant, no API cost)
...
pdfplumber fast path (2 strategies: line-based, text-based)
  ...
  → _read_pdf_as_text()  [regex fallback for CC PDFs]
  → if 0 rows: NVIDIA LLM fallback
```
...
Pass 1: for each email
  ...
  - _try_regex_parse() → raw_txns
  - if empty: extract_transactions_llm() → raw_txns
...
Pure rule-based, instant, no API cost. Applied automatically on every import.

**Rule order:** keyword sets → CC bill → POS fee → ATM → personal UPI → fund transfer → insurance → govt → tech → food extras → fuel → retail → BharatPe → positive amount=Income → Uncategorized

To add a merchant: add keyword to the appropriate set in `_RULES`.
...
toggleGlobalAutoSync()   // 30s auto-poll toggle
syncGmailAll(auto)       // auto=true: today+30msgs; auto=false: 30days+500msgs
```
